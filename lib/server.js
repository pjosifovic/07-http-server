  ...
  requestParser.parse(request)
    .then(request => {
      if(request.method === 'GET' && request.url.pathname === '/'){
        ...
        response.body = cowsay.say({'text' : `${request.body}`});
        response.write(`{"content" : "${response.body}"}`);
        logger.log('info', 'POST - Responding with a 200 status code');
        response.end();
        return;
      }
      ...
      if(request.method === 'POST' && request.body === undefined)
        logger.log('info', 'Body is required');
        errorMessage = '{ "error" : "invalid request: body required" }';

      if(request.method === 'POST' && request.body.text === undefined)
        logger.log('info', 'Invalid body.text');
        errorMessage = '{ "error" : "invalid request: text required" }';
